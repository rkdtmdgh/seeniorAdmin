...
// 유효성 검사
function validateId(input) { // 아이디 유효성 검사
	const regExId = /^[a-zA-Z0-9_-]{6,20}$/; // 정규표현식
	const parentEle = input.parentElement;
	let errorEle = parentEle.querySelector(".input_error"); // 에러 메시지 요소 찾기
    
    // 에러 메시지 요소가 없으면 새로 생성
    if (!errorEle) {
	...
	const result = regExId.test(input.value); // 검사
	//console.log(result);
	
	if(input.value === "" || !result) {
		errorEle.textContent = "아이디는 영문대소문자, 숫자, 특수문자('_', '-')를 사용할 수 있으며 6~20자로 입력해 주세요.";
