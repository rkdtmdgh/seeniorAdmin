...
function validateEmpty(input, txt, alertMsg, notViewMsg = false) { // 요소, 텍스트, alert 여부, 에러메세지 요소 표기 여부(false = 노출, ture = 노출X / 검색폼에서 사용)
	const errorMessage = `${applyJosa(txt, '을/를')} 입력해 주세요.`;
...
function validateVideo(input, alertMsg) {
	...
	if(!validateEmpty(input, 'URL 주소', alertMsg)) {
		if ($previewEle.length) $previewEle.remove();
		return false; // 빈값 확인
	} 
    ...
    if(platformInfo) {	    
		// 미리보기 요소가 없으면 생성
    	if (!$previewEle.length) $previewEle = $(`<div class="preview_container">`); // 요소 생성 및 클래스 추가
		...
		if(alertMsg) alert('지원되지 않은 플랫폼이거나 유효하지 않은 URL입니다. \n지원 플랫폼: YouTube, Vimeo, Dailymotion');
		setAddErrorMessage(input, '지원되지 않은 플랫폼이거나 유효하지 않은 URL입니다. 지원 플랫폼: YouTube, Vimeo, Dailymotion');
		return false;
	}
