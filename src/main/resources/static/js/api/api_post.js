...
async function postDiseaseCategoryCreate(formName, nextPage) {
	...
	const errorMessage = `"${input.value}" 질환/질병 분류 등록에 실패했습니다.`;
...
async function postQnaCategoryCreate(formName) {
	...
	const errorMessage = `"${input.value}" 질문 유형 분류 등록에 실패했습니다.`;
...
async function postReportCategoryCreate(formName) {
	...
	const errorMessage = `"${input.value}" 신고 유형 분류 등록에 실패했습니다.`;
	...
	await postIntegSubmit(
		...
		successMessage,
		errorMessage, 																
		'/report/cate_info/category_list_form',
		'content_inner'
	);
}

// 신고 처리 등록
async function postReportResultCreate(formName) {
	const form = document.forms[formName];
	let input;
